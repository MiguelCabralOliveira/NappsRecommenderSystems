...
def _prepare_gnn_edge_data(
    ...
    try:
        ...
        # Filtrar e Mapear IDs (DF já deve estar limpo, mas verificar mapas)
        print("Filtering interactions by ID maps and mapping IDs...")
        df_gnn = interactions_df.copy()
        initial_count = len(df_gnn)
        df_gnn = df_gnn[
            df_gnn[ID_COL_PERSON_GNN].isin(person_id_map) &
            df_gnn[ID_COL_PRODUCT_GNN].isin(product_id_map) &
            df_gnn[INTERACTION_COL_GNN].isin(INTERACTION_TYPE_MAP_GNN)
        ]
        filtered_count = len(df_gnn)
        print(f"Removed {initial_count - filtered_count} interactions due to missing IDs in maps or unmapped interaction types.")
        ...
        if not pd.api.types.is_datetime64_any_dtype(df_gnn[TIMESTAMP_COL_GNN]):
             ...
             df_gnn[TIMESTAMP_COL_GNN] = df_gnn[TIMESTAMP_COL_GNN].dt.tz_localize('UTC')
        else:
        ...
        # (Lógica de preparação de features de aresta - igual ao script anterior)
        # ... [Copiar/Colar a lógica de preparação de features numéricas e categóricas de aresta aqui] ...
        # ... [Certifique-se de usar as constantes _GNN e salvar scalers/vocabs em output_dir_gnn] ...
        ...
        for split, df_split in datasets.items():
            ...
            for relation, group in grouped:
                ...
                if include_edge_features and split in edge_features_data:
                    # (Lógica para extrair features da aresta para esta relação/split - igual ao script anterior)
                    # ... [Copiar/Colar a lógica de extração de features de aresta aqui] ...
            ...
            if include_edge_features and edge_features_split_dict:
                # (Lógica para salvar features de aresta - igual ao script anterior)
                # ... [Copiar/Colar a lógica de salvamento de features de aresta aqui] ...
...
def run_event_preprocessing(
    ...
    # --- Load Raw Event Data ---
    # (Código de carregamento inicial permanece o mesmo)
    try:
    ...
    try:
        if interactions_df.empty:
             ...
             pd.DataFrame(columns=FINAL_INTERACTION_COLUMNS_CSV).to_csv(output_path, index=False)
        else:
            output_dir_csv = os.path.dirname(output_path)
    ...
    try:
        ...
        if not os.path.exists(person_id_map_path):
             print(f"Error: Cannot prepare GNN edges. Person ID map not found at: {person_id_map_path}")
        ...
        print(f"Error during GNN edge data preparation step: {e}")
        traceback.print_exc()
        print("Warning: GNN edge data preparation failed, but intermediate CSV might be saved.")


    print(f"--- Finished Event Data Preprocessing Orchestration (including GNN prep) ---")
