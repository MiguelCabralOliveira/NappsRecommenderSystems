...
def run_training(args):
    ...
    command.extend(['--num-gnn-layers', str(args.num_gnn_layers)]) # Argumento do train.py
    command.extend(['--dropout', str(args.dropout)])             # Argumento do train.py
    command.extend(['--node-emb-dim', str(args.node_emb_dim)])   # Argumento do train.py
    command.extend(['--link-pred-edge', args.link_pred_edge])
    command.extend(['--lr', str(args.lr)])
    command.extend(['--epochs', str(args.epochs)])
    command.extend(['--batch-size', str(args.batch_size)])       # Argumento do train.py
    num_neighbors_str = ",".join(map(str, args.num_neighbors))
    command.extend(['--num-neighbors', num_neighbors_str])       # Argumento do train.py
    command.extend(['--neg-sampling-ratio', str(args.neg_sampling_ratio)]) # Argumento do train.py
...
if __name__ == "__main__":
    ...
    # --- Argumentos em falta adicionados abaixo ---
    ...
    # --- Fim dos argumentos adicionados ---
    ...
    if run_args.config_file:
        ...
        try:
            ...
            # --- ATUALIZAÇÃO: Processar num_neighbors do ficheiro config ---
            if 'num_neighbors' in config_from_file and isinstance(config_from_file['num_neighbors'], str):
                try:
                    config_from_file['num_neighbors'] = [int(n) for n in config_from_file['num_neighbors'].split(',')]
                     ...
                     del config_from_file['num_neighbors'] # Remove a chave inválida
            # --- FIM ATUALIZAÇÃO ---
    ...
    # Sobrepõe com argumentos de linha de comando (agora inclui todos os adicionados)
    ...
    if 'num_neighbors' in override_args and isinstance(override_args['num_neighbors'], str):
         try:
             ...
             # Valida se o número de vizinhos corresponde às camadas (que podem ter sido sobrescritas também)
         ...
         # Caso já tenha sido processado por config file mas o override não foi dado como string
          num_layers_for_validation = override_args.get('num_gnn_layers', default_train_args['num_gnn_layers'])
          if len(override_args['num_neighbors']) != num_layers_for_validation:
